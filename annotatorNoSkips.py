...
def save_annotation(short, long, frameIdx, rect):
  filename = "annotationsNoSkips.json"
...
can_go_to_next_Frame = False
# submit the annotation
def submit_callback():
  global img_annotations, can_go_to_next_Frame
  ...
  can_go_to_next_Frame = True
  dpg.configure_item("next", enabled=True)

def next_img_shortcut():
  global can_go_to_next_Frame

  if not can_go_to_next_Frame:
    return
  ...
  can_go_to_next_Frame = False
  dpg.configure_item("next", enabled=False)
...
with dpg.window(label="Image Window", width=screen_width, height=screen_height, no_collapse=True, no_resize=True, no_close=True, no_move=True, no_title_bar=True, pos=[0, 0]) as window:
  ...
  dpg.add_button(label="[F12] Next Frame", tag="next", enabled=False, callback=next_img_shortcut)
